...
if produtos_lucratividade:
    produtos_lucratividade.sort(key=lambda x: [1], reverse=True)

    top_5_lucratividade = produtos_lucratividade[:5]

    aux_sheet = workbook.create_sheet(title="Auxiliar")
    aux_sheet.cell(row=1, column=1, value="Nome do Produto")
    aux_sheet.cell(row=1, column=1, value="Lucratividade (%)")

    for idx, (nome_produto, lucratividade) in enumerate(top_5_lucratividade, start=2):
